...
export class OrdersService {
  ...
  async createOrder(userId: string, createDto: CreateOrderDto) {
    if (createDto.reference_type !== OrderReferenceType.SUBSCRIPTION) {
      throw new BadRequestException(
        'Hiện tại chỉ hỗ trợ mua Gói thuê bao (SUBSCRIPTION)',
      );
    }
    ...
    let packageInfo;
    try {
      packageInfo = await this.subscriptionPackagesService[
        'subscriptionPackageModel'
      ]
        .findById(createDto.reference_id)
        .exec();
    } catch (e) {
    ...
    const activeSub =
      await this.teacherSubsService.getCurrentSubscription(userId);
    if (activeSub && activeSub.package_id) {
      ...
      if (oldPackageId === createDto.reference_id) {
        ...
        if (oldPackagePrice === 0) {
          throw new BadRequestException(
            'Bạn đang sử dụng Gói Cơ Bản, không thể gia hạn gói này!',
          );
        }
      ...
      // 2. Nếu truyền vào gói khác gói hiện tại
        ...
        if (packageInfo.price > oldPackagePrice) {
          ...
          const refundAmount =
            oldPackagePrice - (oldPackagePrice * deductionPercentage) / 100;
          finalPrice = Math.max(0, packageInfo.price - refundAmount);
        }
        // Hạ cấp gói (Giá mới <= Giá cũ)
        else {
          throw new BadRequestException(
            'Không thể đăng ký gói thấp hơn hoặc bằng gói hiện tại khi gói hiện tại đang hoạt động!',
          );
        }
      }
    } else {
  ...
  async processSepayWebhook(data: any): Promise<void> {
    ...
    if (savedOrder.reference_type === OrderReferenceType.SUBSCRIPTION) {
      const packageInfo = await this.subscriptionPackagesService[
        'subscriptionPackageModel'
      ]
        .findById(savedOrder.reference_id)
        .exec();
