...
import { StoreItem, StoreItemDocument, StoreItemStatus } from './schemas/store-item.schema';
import { StudentInventory, StudentInventoryDocument } from './schemas/student-inventory.schema';
import { RewardBalance, RewardBalanceDocument } from './schemas/reward-balance.schema';
import { QuestionRewardConfig, QuestionRewardConfigDocument } from './schemas/question-reward-config.schema';
import { LessonRewardConfig, LessonRewardConfigDocument } from './schemas/lesson-reward-config.schema';
import { RewardClaimHistory, RewardClaimHistoryDocument } from './schemas/reward-claim-history.schema';
import { QuizSubmission, QuizSubmissionDocument } from '../quizzes/schemas/quiz-submission.schema';

@Injectable()
export class RewardsService {
  constructor(
    @InjectModel(StoreItem.name) private storeItemModel: Model<StoreItemDocument>,
    @InjectModel(StudentInventory.name) private studentInventoryModel: Model<StudentInventoryDocument>,
    @InjectModel(RewardBalance.name) private rewardBalanceModel: Model<RewardBalanceDocument>,
    @InjectModel(QuestionRewardConfig.name) private questionRewardConfigModel: Model<QuestionRewardConfigDocument>,
    @InjectModel(LessonRewardConfig.name) private lessonRewardConfigModel: Model<LessonRewardConfigDocument>,
    @InjectModel(RewardClaimHistory.name) private rewardClaimHistoryModel: Model<RewardClaimHistoryDocument>,
    @InjectModel(QuizSubmission.name) private quizSubmissionModel: Model<QuizSubmissionDocument>,
  ) {}

  //Store Items
  async createStoreItem(teacherId: string, createDto: any) {
    return this.storeItemModel.create({ ...createDto, teacher_id: new Types.ObjectId(teacherId) });
  }

  async updateStoreItem(teacherId: string, itemId: string, updateDto: any) {
    const item = await this.storeItemModel.findOneAndUpdate(
      { _id: new Types.ObjectId(itemId), teacher_id: new Types.ObjectId(teacherId) },
      updateDto,
      { new: true }
    );
    if (!item) throw new NotFoundException('Store item not found or you do not have permission');
    return item;
  }
  ...
  async getActiveStoreItems() {
    return this.storeItemModel.find({ status: StoreItemStatus.ACTIVE, stock: { $gt: 0 } }).exec();
  }
  ...
  async getBalance(studentId: string) {
    const record = await this.rewardBalanceModel.findOne({ student_id: new Types.ObjectId(studentId) }).exec();
  ...
  async redeemItem(studentId: string, itemId: string) {
    ...
    const balanceRecord = await this.rewardBalanceModel.findOne({ student_id: studentObjId });
  ...
  async getStudentInventory(studentId: string) {
    return this.studentInventoryModel.find({ student_id: new Types.ObjectId(studentId) }).populate('item_id').exec();
  }
  ...
  async bulkConfigureQuestions(teacherId: string, items: { question_id: string, reward_points: number }[]) {
    const operations = items.map(item => ({
      updateOne: {
        filter: { question_id: new Types.ObjectId(item.question_id) },
        update: { $set: { reward_points: item.reward_points, teacher_id: new Types.ObjectId(teacherId) } },
        upsert: true
      }
    }));
    await this.questionRewardConfigModel.bulkWrite(operations as any);
    return { success: true, count: items.length };
  }

  //Lesson Reward Config
  async configureLessonReward(teacherId: string, lessonId: string, rewardPoints: number) {
    return this.lessonRewardConfigModel.findOneAndUpdate(
      { lesson_id: new Types.ObjectId(lessonId) },
      { reward_points: rewardPoints, teacher_id: new Types.ObjectId(teacherId) },
      { new: true, upsert: true }
    );
  }
  ...
  async claimQuizReward(studentId: string, submissionId: string) {
    ...
    const submission = await this.quizSubmissionModel.findOne({ _id: subObjId, student_id: studentObjId }).exec();
    if (!submission) {
    ...
    if (correctQuestionIds.length === 0) {
      await this.rewardClaimHistoryModel.create({ student_id: studentObjId, reference_id: subObjId, reference_type: 'QUIZ' });
      return { rewarded_points: 0 };
    }

    const configs = await this.questionRewardConfigModel.find({ question_id: { $in: correctQuestionIds } }).exec();
    
    const totalPoints = configs.reduce((sum, config) => sum + config.reward_points, 0);
  ...
  async claimLessonReward(studentId: string, lessonId: string) {
    ...
    const config = await this.lessonRewardConfigModel.findOne({ lesson_id: lessonObjId });
