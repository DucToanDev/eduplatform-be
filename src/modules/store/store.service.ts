...
export class StoreService {
  ...
  async redeemItem(studentId: string, itemId: string) {
    ...
    if (!item || (item.status as unknown as StoreItemStatus) !== StoreItemStatus.ACTIVE || item.stock <= 0) {
      throw new BadRequestException('Item is not available');
    }

    const balanceRecord = await this.pointsService.getBalance(studentId);
    ...
    // Deduct points
    await this.pointsService.adjustPoints(studentId, -item.points);
