...
export class QuizzesService {
  ...
  private async checkQuizOwnershipByQuizDoc(quiz: Quiz, authorId: string) {
    if (quiz.quiz_type === QuizType.COURSE_EXAM) {
      if (!quiz.course_id)
        throw new BadRequestException('course_id là bắt buộc');
      await this.lessonsService.checkClassOwnership(
        quiz.course_id.toString(),
        authorId,
      );
    } else {
      if (!quiz.lesson_id)
        throw new BadRequestException('lesson_id là bắt buộc');
      const lesson = await this.lessonsService.findOne(
        quiz.lesson_id.toString(),
      );
      await this.lessonsService.checkClassOwnership(
        lesson.class_id.toString(),
        authorId,
      );
    }
  }
  ...
  async createQuiz(
    ...
    if (createQuizDto.quiz_type === QuizType.COURSE_EXAM) {
      if (!createQuizDto.course_id)
        throw new BadRequestException(
          'course_id là bắt buộc đối với COURSE_EXAM',
        );
      await this.lessonsService.checkClassOwnership(
      ...
      if (!createQuizDto.lesson_id)
        throw new BadRequestException(
          'lesson_id là bắt buộc đối với LESSON_QUIZ',
        );
  ...
  async getQuizzesByCourse(courseId: string): Promise<Quiz[]> {
    return this.quizModel
      .find({ course_id: courseId, quiz_type: QuizType.COURSE_EXAM })
      .sort({ order_index: 1 })
      .exec();
  }
  ...
  async getQuizzesByLesson(lessonId: string): Promise<Quiz[]> {
    return this.quizModel
      .find({ lesson_id: lessonId, quiz_type: QuizType.LESSON_QUIZ })
      .exec();
  }
  ...
  async getQuestionBankByCourse(
    ...
    const allQuizIds = [
      ...courseQuizzes.map((q) => q._id),
      ...lessonQuizzes.map((q) => q._id),
    ];
  ...
  async updateQuestion(
    ...
    const updated = await this.questionModel
      .findByIdAndUpdate(id, updateQuestionDto, {
  ...
  async submitQuiz(
    ...
    const submissionAnswers = answers.map((ans) => {
      ...
      const is_correct = q
        ? q.correct_option_index === ans.selected_index
        : false;
