...
export class LearningRewardsService {
  ...
  async claimQuizReward(studentId: string, submissionId: string) {
    ...
    const session = await this.rewardClaimHistoryModel.db.startSession();
    session.startTransaction();
    try {
      const existingClaim = await this.rewardClaimHistoryModel
        .findOne({
      ...
      await this.pointsService.adjustPoints(studentId, totalPoints, session);
  ...
  async claimLessonReward(studentId: string, lessonId: string) {
    ...
    try {
      ...
      const config = await this.lessonRewardConfigModel
        .findOne({
